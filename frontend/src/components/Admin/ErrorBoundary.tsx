...
import { useVerifyARefreshTokenMutation } from "../../utils/redux/slices/adminApiSlices"
...
export const ErrorBoundary: React.FC<ChildrenProps> = ({ children }) => {
    ...
    const [verifyARefreshToken] = useVerifyARefreshTokenMutation()
    ...
    const handleTokenRefresh = async () => {
        try {
            setLoading(true)
            const refresh = await verifyARefreshToken(undefined)
            console.log(refresh)
            if(refresh?.data){
                setLoading(false)
                setError(null)
